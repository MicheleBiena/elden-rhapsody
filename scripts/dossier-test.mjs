...
try {
  ...
  assert.match(await page.locator('.live-update-highlight').textContent(), /Aggiunto ora.*Chiesa del Pellegrinaggio/i)
  assert.equal(await page.locator('.dossier-text-section.is-highlighted').count(), 1)
  assert.match(await page.locator('.dossier-text-section.is-highlighted').textContent(), /Aggiunto ora.*Lord Godfrey/is)
  ...
  assert.equal(await page.locator('.live-update-highlight').count(), 1)
  assert.match(await page.locator('.live-update-highlight').textContent(), /Aggiunto ora.*seconda Sellen/i)
